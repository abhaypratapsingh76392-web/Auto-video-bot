...
final_video_name = f"video_USA_{today_date}.mp4"
...
try:
    ...
    print(f"✅ Gemini generated {len(scenes)} scenes successfully!")
...
for i, scene in enumerate(scenes):
    ...
    if img_response.status_code == 200:
        ...
        print(f"⚠️ Image download failed for scene {i+1}. Skipping.")
        continue 
    ...
    ffmpeg_cmd = (
        ...
        f'-vf "zoompan=z=\'min(zoom+0.0015,1.2)\':d=1000:x=\'iw/2-(iw/zoom/2)\':y=\'ih/2-(ih/zoom/2)\':s=1080x1920" '
        f'-c:v libx264 -pix_fmt yuv420p -c:a aac -shortest "{clip_file}" -loglevel error'
    )
...
<body>
    ...
    <div class="grid">
        {"".join([f'<div class="vid-card"><span class="date-label">📅 {v.replace("video_USA_", "").replace(".mp4", "")}</span><video src="{v}" controls preload="metadata"></video><a href="{v}" download class="download-btn">⬇️ Download High Quality</a></div>' for v in all_videos]) or "<h3>No videos generated yet.</h3>"}
    </div>
</body>
</html>
"""
